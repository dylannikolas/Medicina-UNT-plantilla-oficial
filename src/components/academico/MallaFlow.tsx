...
const AREA_STYLES: Record<AreaType, string> = {
  ...
  especifico:
    "border-l-orange-400 bg-white text-orange-950 hover:bg-orange-100/70",
  ...
  return (
    ...
    >
      <Handle
        ...
        className="opacity-40 !bg-slate-400"
      />
      ...
      <div className="mt-2 pt-1.5 border-t border-slate-200/50 text-[9px] opacity-80 flex justify-between items-center">
        <span>
          HT: {data.hoursT}h | HP: {data.hoursP}h
        </span>
...
const PLAN_PDF = `${import.meta.env.BASE_URL}PLAN-DE-ESTUDIOS-CURRICULO-2018.pdf`;
...
export default function MallaFlow() {
  ...
  const openCourse = useCallback(
    (course: CourseData) => setSelectedCourse(course),
    [],
  );
  ...
  const initialNodes = useMemo<Node<CourseNodeData>[]>(() => {
    return CURRICULUM_DATA.map((course) => {
      ...
      return {
        ...
        type: "courseNode",
        position: {
          x: CYCLE_COLUMNS[course.cycle] ?? 0,
          y: orderInCycle * 110,
        },
      ...
      <div className="grid grid-cols-2 gap-3 bg-slate-50 p-3 rounded-xl border border-slate-200/60 text-xs mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="font-bold text-slate-700">
              HT {course.hoursT}h · HP {course.hoursP}h
            </p>
          </div>
        </div>
      </div>
  ...
  const overview = (
    <>
      <h3 className="font-display font-black text-primary text-lg leading-tight mb-2">
        Malla curricular
      </h3>
      <p className="text-sm text-slate-600 leading-relaxed mb-5">
        Mapa interactivo del plan de estudios por ciclos. Selecciona un curso
        para ver sus créditos, horas y descripción. Las líneas conectan los
        prerrequisitos.
      </p>
      ...
      <div className="space-y-2 text-xs">
        <div className="flex items-center gap-2">
          <span className="w-3.5 h-3.5 bg-emerald-50 border border-emerald-400 rounded shrink-0" />
          <span className="text-slate-700">Estudios Generales</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-3.5 h-3.5 bg-orange-50 border border-orange-400 rounded shrink-0" />
          <span className="text-slate-700">Estudios Específicos</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-3.5 h-3.5 bg-sky-50 border border-sky-400 rounded shrink-0" />
          <span className="text-slate-700">Estudios de Especialidad</span>
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <div className="sr-only">
        <table>
          <caption>
            Plan de estudios por ciclos: {stats.cycles} ciclos, {stats.courses}{" "}
            cursos, {stats.credits} créditos.
          </caption>
          <thead>
            <tr>
      ...
      <div className="lg:flex lg:items-stretch rounded-xl border border-slate-200 overflow-hidden shadow-sm bg-white">
        ...
        <div className="relative w-full lg:flex-1 h-[70svh] min-h-[460px] md:h-[720px] bg-slate-50 overflow-hidden lg:border-r lg:border-slate-200">
          ...
          <div className="absolute bottom-3 right-3 z-10 flex items-end gap-2">
            {showHelp && (
              <div className="bg-slate-900/90 text-white text-[10px] sm:text-[11px] px-2.5 py-1.5 rounded-lg shadow-md max-w-[230px]">
                Arrastra para mover
                {isDesktop
                  ? " · Ctrl + rueda para acercar/alejar"
                  : " · pellizca para zoom"}
                . {isDesktop ? "Haz clic en" : "Toca"} un curso para ver su
                detalle.
              </div>
            )}
            <button
          ...
          <ReactFlow
            ...
            fitView={isDesktop}
            defaultViewport={
              isDesktop ? undefined : { x: 16, y: 16, zoom: 0.85 }
            }
            minZoom={0.3}
            maxZoom={1.5}
      ...
      <div className="lg:hidden mt-3 flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-xs text-slate-600">
        ...
        <span className="flex items-center gap-1.5">
          <span className="w-3.5 h-3.5 bg-emerald-50 border border-emerald-400 rounded" />{" "}
          Estudios Generales
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-3.5 h-3.5 bg-orange-50 border border-orange-400 rounded" />{" "}
          Estudios Específicos
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-3.5 h-3.5 bg-sky-50 border border-sky-400 rounded" />{" "}
